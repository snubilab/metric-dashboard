...
export const segmentationScenariosKo: Scenario[] = [
  {
    ...
    clinical: {
      situation:
        ...
        "병변이 보입니다. 모델은 큰 병변은 훌륭하게 분할하지만 작은 병변은 통째로 " +
        "놓칩니다.",
      modality: "조영증강 뇌 MRI",
      atStake:
        ...
        "있습니다.",
      consequence:
        "복셀 Dice는 훌륭해 보이고 리포트도 검토를 통과하지만, 정작 치료할 수 있는 " +
        "병변이 끝내 표시되지 않았습니다.",
    },
    state: stateFor("missed-met"),
    teachingPoint:
      "큰 병변이 복셀 수를 좌우하다 보니 복셀 Dice는 0.85 이상으로 유지됩니다. 그러나 " +
      "두 병변 중 하나만 찾았기 때문에 병변 단위 민감도는 0.5에 그칩니다. 병변 단위 " +
      "지표는 복셀 Dice가 가려 버린, 놓친 전이를 드러냅니다.",
  ...
  },
  {
    ...
    clinical: {
      ...
      modality: "조영증강 복부 CT",
      atStake:
        "경계 지표는 실제 병변의 절제연(margin) 정확도를 보증하는 역할을 맡아야 " +
        "합니다.",
      consequence:
  ...
  },
  {
    ...
    clinical: {
      situation:
        "종양을 절대 놓치지 않도록 튜닝된 모델이라, 실제 종양보다 훨씬 넓은 영역을 " +
      ...
      consequence:
        "민감도는 거의 완벽하지만 정밀도는 무너집니다. 예측한 픽셀 대부분이 " +
        "위양성이기 때문입니다.",
    },
    state: stateFor("over-segmentation"),
    teachingPoint:
      "예측 A는 정답 전체를 덮지만(민감도 ≈ 1) 면적이 훨씬 커서 정밀도가 낮습니다. " +
      "민감도만 보면 과다 분할을 오히려 잘한 것으로 쳐 주게 되니, 정밀도와 반드시 함께 읽어야 합니다.",
  ...
  },
  {
    ...
    clinical: {
      situation:
        "간 종양 마스크가 전반적으로는 정답과 잘 겹치지만, 외과의가 정확한 절제연을 " +
        "요구하는 바로 그 부위에서 경계의 한쪽 엽(lobe)이 바깥으로 부풀어 있습니다.",
      modality: "조영증강 복부 CT (수술 계획)",
      atStake:
        "절제연은 경계에서 곧장 읽어내기 때문에, 국소적인 돌출은 너무 많이 잘라내거나 " +
        "너무 적게 잘라낸다는 뜻이 됩니다.",
      consequence:
    ...
    teachingPoint:
      "부풀어 오른 절제연은 연속된 픽셀 덩어리를 더하므로 Dice는 높게 유지됩니다. 그러나 HD95는 " +
      "돌출의 깊이만큼 올라갑니다. 수술 절제연에서는 경계 지표를 타협할 수 없습니다. Dice " +
      "하나만 믿으면 엉뚱한 마스크를 통과시키게 됩니다.",
  ...
  },
  {
    ...
    clinical: {
      situation:
        "두 후보 모델을 비교합니다. Dice로는 거의 동점이라, Dice만 보는 리더보드라면 " +
        "사실상 동전 던지기로 우열을 가리는 셈입니다.",
    ...
    teachingPoint:
      "두 예측 모두 면적이 같은 작은 돌기를 더하므로 Dice 차이는 무시할 만합니다. 그런데 " +
      "B의 돌기는 훨씬 바깥에 있어 HD95가 훨씬 큽니다. 겹침 순위가 같아도 그 뒤에 큰 경계 품질 " +
      "격차가 숨어 있을 수 있습니다.",
  ...
  },
  {
    ...
    clinical: {
      situation:
        "정말로 정상인 검사라 대상 병변이 하나도 없는데도, 모델이 엉뚱한 검출 하나를 " +
        "내놓습니다.",
  ...
  },
  {
    ...
    clinical: {
      situation:
        "혈관 분할 모델이 픽셀 겹침으로는 좋은 마스크를 만들어 내지만, 정작 중심선(centerline)이 " +
        "끊겨 한 분지가 두 조각으로 갈라져 있습니다.",
      modality: "CT 혈관조영 (관 모양 구조)",
      atStake:
        "혈관과 기도에서는 연결성(connectivity)이 중요합니다. 중심선이 끊겨 있으면 실제로는 " +
        "없는 폐색(occlusion)이 있는 것처럼 비칠 수 있습니다.",
      consequence:
        "대부분의 픽셀이 겹치므로 Dice와 IoU는 멀쩡해 보입니다. 그러나 위상은 틀렸고, 이는 " +
        "겹침 지표로는 볼 수 없는 부분입니다.",
    },
    state: stateFor("broken-vessel-topology"),
    teachingPoint:
      "예측 A는 혈관 한가운데에 작은 틈을 남깁니다. 복셀 Dice는 거의 영향을 받지 않지만, " +
      "구조는 이제 끊어진 두 조각이 됩니다. clDice / 중심선 Dice 같은 위상 인식(topology-aware) " +
      "지표는 겹침 지표가 놓치는 바로 이 단절을 잡아내려고 설계된 것입니다.",
