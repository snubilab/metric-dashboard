/**
 * DetectionMetricsPanel — the controlled metric body of the detection board.
 *
 * Extracted from DetectionBoard so a playground can OWN the confidence threshold
 * (and share it with a canvas) while reusing the vetted metric logic. The panel
 * holds NO threshold/apMethod state of its own: it receives them as props plus
 * change callbacks, making it fully controlled.
 *
...
export interface DetectionMetricsPanelProps {
  ...
  iouThreshold: number;
  /** Operating confidence threshold (controlled). */
  threshold: number;
  /** AP interpolation method (controlled). */
  apMethod: ApMethod;
  /** Called when the user moves the confidence-threshold slider. */
  onThresholdChange: (threshold: number) => void;
  /** Called when the user picks a different AP interpolation method. */
  onApMethodChange: (method: ApMethod) => void;
}
...
export function DetectionMetricsPanel({
  gt,
  preds,
  iouThreshold,
  threshold,
  ...
  return (
    ...
    >
      <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-6)", alignItems: "flex-end" }}>
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
          ...
          <select
            ...
            value={apMethod}
            onChange={(e) => onApMethodChange(e.target.value as ApMethod)}
            style={{
              fontFamily: "var(--font-ui)",
      ...
      <div style={{ display: "flex", gap: "var(--space-8)" }}>
        <Count tone="--c-gt-text" label="TP" value={counts.tp} dataCount="tp" />
        <Count tone="--c-warn-text" label="FP" value={counts.fp} dataCount="fp" />
